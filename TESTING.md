...
| Settings page / UI themes (theme + accent name-to-resource mapping) | `AstraThemeCatalogTests` | Each known theme name resolves to its expected `Themes/*.xaml` path; "Playnite" and any unrecognized theme name resolve to `null` (meaning "no override dictionary"); a known accent name resolves to its exact hex; an unrecognized accent name falls back to the first (Teal) rather than throwing. The WPF-only half (`AstraThemeApplier`, actually merging/removing the dictionary and surfacing load errors) is not unit-testable - same documented gap as `ShareCardRenderer`/`TrendChartControl` - verified manually only |
